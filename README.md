...
- 运行全部测试：npm test（当前 573 项全部通过，覆盖存储/加密（含竞态与未来版本韧性）/结构化存储状态（TrustLayer 契约）/标准模块工厂（ModuleFactory 契约与行为，含 v0.1.1 prepend/timeField 扩展）/事件总线契约（EventBridge：EVENT 常量表与 payload 契约）/UI/各模块/样式/动效/壁纸/移动端自动适配/性能/游戏引擎/交互质量回归/PWA/搜索/XSS 净化（含属性注入）/对比度/IndexedDB/通知/Web Worker/持久化危机兜底/动态交互层契约与行为/file:// 直开环境/ESM 试验田双实现一致性）。
