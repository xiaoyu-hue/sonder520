# 🌊 Sonder520

> 一款个人工作与生活管理工具
> 
> 纯前端 · 零依赖 · 本地优先 · 水墨液态玻璃风格

**一个人、一台电脑、浏览器就够了。** 数据完全存储在本地浏览器，无需注册、无需联网、无需付费。

---

<div align="center">

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Release](https://img.shields.io/github/v/release/xiaoyu-hue/sonder520?style=flat)
![Last Commit](https://img.shields.io/github/last-commit/xiaoyu-hue/sonder520?style=flat)
![MIT](https://img.shields.io/badge/License-MIT-yellow.svg)
![PWA](https://img.shields.io/badge/PWA-Support-green)
![Zero-Dependency](https://img.shields.io/badge/Zero-Dependency-6B728C)
![CI](https://github.com/xiaoyu-hue/sonder520/actions/workflows/deploy.yml/badge.svg)
![Tests](https://img.shields.io/badge/tests-770_passing-2ea44f?style=flat)

</div>

---

**[🚀 在线体验](https://sonder520.pages.dev/)** · **[📖 使用文档](#-使用)** · **[🛠️ 技术架构](#-技术架构)** · **[📦 本地运行](#-本地运行)**

---

## 🎯 项目简介

**Sonder520** 是一款**零后端、纯前端**的个人工作与生活管理工具，由 AI Agent 辅助开发。项目以「水墨 × 液态玻璃」为视觉语言，将效率工具与东方美学融合。

### ✨ 核心特点

| 特性 | 说明 |
|------|------|
| 🌐 **纯前端** | HTML/CSS/原生 JavaScript，零构建、零运行时依赖 |
| 🔒 **隐私优先** | 数据仅存于本地浏览器，支持可选加密 |
| 🎨 **水墨风格** | 宣纸/墨黑双主题，液态玻璃卡片设计 |
| 📱 **浏览器内三端响应式** | 桌面/平板/手机三端响应式布局，PWA 离线可用 |
| 🎮 **内置游戏** | 井字棋、五子棋、扫雷等 6 款迷你游戏 |
| 🧪 **测试覆盖** | 770 项测试通过（单元测试 + 契约测试 + 集成测试 + E2E） |

### 🔐 数据与隐私

- **数据只存在你的浏览器里**：主存储是 IndexedDB（真源），localStorage 用作副本和跨标签页信号；数据不会上传到任何服务器。
- **可选加密**：可在设置中开启加密存储（PBKDF2 派生密钥 + AES-GCM-256），加密后即使有人拿到浏览器数据也无法读取。
- **备份靠手动**：通过「设置 → 导出备份」下载 JSON 备份文件；清除浏览器数据或更换设备前，请先备份。
- **零运行时依赖**：项目不依赖任何第三方运行时库，所有功能由浏览器原生 API 实现。

### ⚠️ 不适合什么场景

诚实说，Sonder520 不是万能工具，以下场景请谨慎使用或选择更专业的方案：

- **需要跨设备自动同步** — 数据只存在当前浏览器，更换设备或清除浏览器数据后无法自动恢复（需手动导入备份）
- **需要团队协作** — 个人工具，没有多人共享、协作或权限管理功能
- **需要原生 App 体验** — 是 Web App（PWA），不是 iOS/Android 原生应用，部分系统级能力（如后台推送、系统级通知）受限
- **重要数据唯一存储** — 浏览器数据可能被清理或损坏，建议定期导出备份，不要把唯一副本放在浏览器里

---

## 📋 功能特性

### 12 个核心模块

| 模块 | 功能 | 特色 |
|------|------|------|
| 🏠 **首页** | 问候语 + 每日金句 + 任务概览 | 完成率环形图、快速备忘 |
| 📅 **今日计划** | 四档优先级、番茄钟、分组管理 | 25 分钟专注计时器 |
| 📝 **快速备忘** | 即时记录、历史存档 | 快速保存、一键归档 |
| 📱 **自媒体** | 多平台排期、发布统计 | 微信/小红书/B站/抖音 |
| 💻 **开发工作** | 项目任务、技术笔记 | Markdown 渲染、代码高亮 |
| 🤝 **咨询服务** | 客户管理、收入记录 | 阶段跟进、收入统计 |
| 📚 **阅读** | 书单管理、阅读计时、书摘 | 进度追踪、读书分享 |
| 📰 **资讯** | 文章收藏、未读管理 | 链接跳转、分类整理 |
| 🎨 **设计** | 灵感收集、设计项目 | 阶段性管理 |
| 🎮 **娱乐** | 6 款迷你游戏 | 井字棋、五子棋、扫雷等 |
| 📊 **数据统计** | 多维度数据可视化 | 完成率、进度图表 |
| ⚙️ **设置** | 主题、壁纸、加密、备份 | 个性化定制 |

---

## 🛠️ 技术架构

### Sonder-Frame 渐进式框架

> 注：Sonder-Frame 是本项目内部自研的轻量框架，非通用开源框架，专为个人工具类应用设计。

```
Application (应用层 · 10 页面)
    ↓
Domain (领域规则层 · task-domain 纯函数)
    ↓
Repository (数据访问边界 · 9 个薄包装)
    ↓
ModuleFactory + Store 领域文件 (标准模块工厂 + 10 领域扩展)
    ↓
VisualEngine + EventBridge (UI渲染 + 事件总线)
    ↓
TrustLayer (安全存储层 · 写锁收口 ADR-013)
    ↓
IDB (主存储) + localStorage (副本) + Crypto (加密)
```

> v6.x 架构升级：数据层拆出 Repository 数据边界与 Domain 规则层；store.js 按职责拆为 10 个领域扩展文件（详见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)）。

### 核心技术栈

| 技术 | 用途 |
|------|------|
| **HTML5** | 语义化结构 |
| **CSS3** | 液态玻璃设计、水墨风格、响应式布局 |
| **原生 JavaScript** | 零运行时依赖、零构建步骤，直接在浏览器运行 |
| **IndexedDB** | 主数据存储（真源） |
| **localStorage** | 副本存储 + 跨标签信号 |
| **Crypto API** | PBKDF2 + AES-GCM-256 加密 |
| **Web Worker** | 五子棋 AI 异步计算 |
| **PWA** | 离线可用、安装到桌面 |
| **Playwright** | E2E 测试（三端：桌面/平板/手机） |

### 质量保障

- ✅ **770 项测试**通过（单元测试 + 契约测试 + 集成测试 + E2E）
- ✅ **零构建**类型检查（JSDoc + TypeScript）
- ✅ **ESLint** 代码规范
- ✅ **14 份 ADR** 架构决策记录
- ✅ **PWA** 离线支持 + 版本更新
- ✅ **架构分层**：Domain 规则层 + Repository 数据边界 + Store 领域扩展（10 文件）

---

## 🎨 视觉与交互

### 水墨 × 液态玻璃设计语言

- **双主题**：宣纸（浅色）/ 墨黑（深色），跟随系统
- **液态玻璃卡片**：`backdrop-filter` 毛玻璃效果 + 内部高光
- **朱砂红点缀**：中国传统色作为强调色
- **中国颜料配色**：图表使用传统色谱

### 微交互

- 交错淡入动画
- 水墨扩散图表动画
- 按钮按压反馈
- 呼吸感空状态
- 滑动 Toast 提示
- 移动端底部弹窗

### 对比度合规

- 所有文本对比度 ≥4.5:1（WCAG AA 标准）
- 扫雷数字在两种主题下均清晰可读
- 测试锁定对比度 token

---

## 📱 平台适配

| 设备 | 布局 |
|------|------|
| **桌面（>960px）** | 左侧液态玻璃侧边栏 + 多列网格 |
| **平板（721-960px）** | 侧边栏折叠为 70px 图标栏 |
| **手机竖屏（≤720px）** | 底部液态玻璃导航栏（可滚动图标） |
| **超小屏（≤360px）** | 压缩间距，导航不折断 |
| **手机横屏** | 精简顶部栏和导航 |

### 移动端优化

- ✅ iOS/Android 适配
- ✅ viewport-fit=cover（刘海屏适配）
- ✅ safe-area inset（手势栏适配）
- ✅ 100vh→100dvh 回退
- ✅ 触控目标 ≥44px
- ✅ 16px 输入防 iOS 缩放

---

## 🚀 快速开始

### 在线使用（推荐）

访问 https://sonder520.pages.dev/ 立即使用，无需安装。

### 本地运行

1. 克隆仓库：
```bash
git clone https://github.com/xiaoyu-hue/sonder520.git
cd sonder520
```

2. 直接在浏览器打开 `index.html`（双击即可）

> 💡 推荐 Chrome / Edge 浏览器

### 数据管理

- 数据存储在浏览器本地（IndexedDB 主存储 + localStorage 副本）
- 定期通过「数据与设置 → 导出备份」下载 JSON 备份
- 可在设置中开启加密存储（PBKDF2 + AES-GCM）

---

## 📦 本地运行与开发

### 环境要求

- Node.js 18+
- npm 9+

### 开发命令

```bash
# 安装依赖
npm install

# 运行所有测试
npm test

# 类型检查（零构建）
npm run typecheck

# 代码规范检查
npm run lint

# E2E 测试（Playwright）
npm run test:e2e

# 同步 Service Worker 缓存
npm run sync-sw
```

### 测试覆盖

- **单元测试**：存储、加密、TrustLayer、Domain 规则
- **契约测试**：ModuleFactory CRUD、Repository 数据边界、type-sync
- **集成测试**：Factory+TrustLayer、Factory+VisualEngine、Module+EventBridge
- **E2E 测试**：Playwright 三端（桌面/平板/手机）

---

## 📚 文档

- **[PRD.md](docs/PRD.md)** - 产品需求文档（中文版，含版本历史、视觉设计、数据规范、验收标准）
- **[ARCHITECTURE.md](docs/ARCHITECTURE.md)** - 当前技术架构一页图（分层/文件职责/加载顺序/红线）
- **[AGENTS.md](AGENTS.md)** - 项目协作规则（AI Agent 开发规范）
- **[CHANGELOG.md](CHANGELOG.md)** - 更新日志
- **[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)** - 行为准则
- **[LICENSE](LICENSE)** - MIT 许可证
- **[docs/](docs/README.md)** - 文档总索引（ADR 决策记录、迁移计划、规格与验收）
- **[AUTHOR.md](docs/AUTHOR.md)** - 关于作者

---

## 📈 更新日志

### v6.3.1（当前版本）

- **v6.x 架构升级**：Repository 数据边界（9 个）+ Domain 规则层（task-domain）+ store.js 职责拆分（10 领域扩展文件）
- **UX 修复**：状态下拉中文化、移动端日期时间、顶部时钟秒级跳动

### v6.0（架构底座与视觉）

### 历史版本

- v5.2 - 工程防线补强 + 可靠性提升
- v5.1 - 性能与可靠性四补丁
- v5.0 - 统计语义修正 + CSV 注入防护
- v4.0 - 质量加固体系
- v3.0 - 可选加密 + 四款新游戏
- v2.1 - 游戏/性能/PWA/安全/存储升级
- v2.0 - 游戏交互质量回归
- v1.3 - 手机适配 + 每日金句
- v1.2 - 水墨微动态交互
- v1.1 - 水墨液态玻璃视觉
- v1.0 - 初始化

---

## 🤝 贡献指南

欢迎贡献！请阅读 [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) 和 [AGENTS.md](AGENTS.md) 了解项目规范。

### 贡献流程

1. Fork 本仓库
2. 创建特性分支（`git checkout -b feature/xxx`）
3. 提交更改（`git commit -m '添加xxx功能'`）
4. 推送到分支（`git push origin feature/xxx`）
5. 创建 Pull Request

---

## 📄 许可证

本项目采用 [MIT 许可证](LICENSE) - 免费使用、修改和分发。

---

## 🙏 致谢与依赖

> **"If I have seen further, it is by standing on the shoulders of giants."**
> 我看得更远，是因为站在巨人的肩膀上。—— 送给支撑 Sonder520 的开源社区、浏览器标准与 AI Agent。

Sonder520 站在这些开源项目和浏览器标准的肩膀上。没有它们，一个零编程基础的作者不可能做出这个项目。

### 开发与测试工具链

| 项目 | 协议 | 说明 |
|------|------|------|
| [Node.js](https://nodejs.org) | MIT | 开发/测试运行时（node --test、tsc、eslint、playwright 全部跑在它上面） |
| [npm](https://www.npmjs.com) | Artistic-2.0 | 包管理与脚本执行（零运行时依赖，仅开发期使用） |
| [Playwright](https://playwright.dev) | Apache-2.0 | E2E 测试（桌面/平板/手机三端） |
| [ESLint](https://eslint.org) | MIT | 代码规范检查 |
| [eslint-plugin-jsdoc](https://github.com/gajus/eslint-plugin-jsdoc) | BSD-3-Clause | JSDoc 注释规范检查 |
| [TypeScript](https://www.typescriptlang.org) | Apache-2.0 | 零构建类型检查（通过 JSDoc 注释） |
| [jsdom](https://github.com/jsdom/jsdom) | MIT | DOM 测试环境 |
| [fake-indexeddb](https://github.com/dumbmatter/fakeIndexedDB) | Apache-2.0 | IndexedDB 测试 mock |

### 架构思想与设计借鉴

Sonder520 是零依赖自研实现，但架构并非凭空发明——它站在大量开源思想与工程实践的肩膀上，**借鉴的是模式与标准，代码全部自研**。

| 借鉴来源 | 思想/模式 | 在本项目中的体现 |
|------|------|------|
| [Martin Fowler《企业应用架构模式》](https://martinfowler.com/eaaCatalog/repository.html) | Repository 模式（数据访问边界） | `js/repositories/` 9 个薄包装 Repository，统一 get/getAll/create/update/remove，页面不直连存储细节 |
| [Clean Architecture（Robert C. Martin）](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html) | 分层架构、依赖向内 | UI → Domain → Repository → Persistence 的依赖方向，边界清晰可测 |
| [Domain-Driven Design 战术分层](https://martinfowler.com/bliki/AnemicDomainModel.html)（Eric Evans） | 领域模型与存储/UI 分离 | `js/domain/task-domain.js` 纯函数规则层（任务完成/重开语义），不碰 DOM 与存储 |
| 前端组件化与事件驱动生态（各框架的模块系统与发布-订阅模式） | 模块工厂 + 事件总线 | ModuleFactory 标准模块协议 + SonderBus 事件总线（store 变更 → 页面自动重绘） |
| [Ink & Switch《Local-First Software》](https://www.inkandswitch.com/local-first/) 与本地优先社区 | 本地设备为主数据副本、离线可用 | IndexedDB 真源 + localStorage 副本 + 双写双存 + PWA 离线 |
| [TypeScript 团队](https://www.typescriptlang.org) | JSDoc 类型注解 + tsc 零构建检查 | `globals.d.ts` + `tsc --noEmit`，无编译步骤即得类型契约 |
| 社区通用模块规范 | UMD（Universal Module Definition） | 全部领域/Repository/Domain 文件浏览器 + Node 双环境可加载 |
| [W3C / MDN 开放标准](https://developer.mozilla.org) | WebCrypto（PBKDF2 + AES-GCM-256）、IndexedDB、Web Worker、Service Worker、backdrop-filter、CSS 自定义属性 | 加密、持久化、AI Worker、PWA、液态玻璃视觉全部基于浏览器原生能力 |

### 视觉与设计灵感

- **液态玻璃（Glassmorphism）**：磨砂玻璃卡片、高光内边、通透层次来自社区流行的 Glassmorphism 设计运动（[glassmorphism.com](https://glassmorphism.com) 与众多 UI 设计社区实践）。
- **水墨美学**：宣纸/墨黑双主题、朱砂红强调、国画颜料色谱来自中国传统水墨艺术——这是文化的馈赠，不是开源代码，同样心怀敬意。

### 部署与 CI 平台

| 平台 | 说明 |
|------|------|
| [Cloudflare Pages](https://pages.cloudflare.com) | 免费静态托管（主站 https://sonder520.pages.dev/） |
| [Netlify](https://www.netlify.com) | 免费静态托管（镜像站 https://sonder520.netlify.app/） |
| [GitHub Actions](https://github.com/features/actions) | 免费 CI：测试/类型检查/lint/SW 指纹门禁 + E2E 三端矩阵 |

### 特别致谢

- **浏览器原生能力**（详见上表 W3C/MDN 标准行）：Sonder520 零运行时依赖，所有功能基于浏览器原生 API 实现。
- **浏览器厂商**：Chromium（Chrome/Edge）、Firefox（Gecko）、WebKit（Safari）团队——WebCrypto、backdrop-filter、PWA 等标准由他们实现并推进，写标准的 W3C/MDN 与实现标准的厂商缺一不可。
- **AI Agent**：项目开发由 AI Agent 辅助完成
- **所有为开源社区贡献代码、文档和时间的人。**

> 一句话总结：**借鉴的是模式与标准，代码全部自研；依赖是零，敬意是满的。**

---

**Made with 💧 水墨风格**

> "每个人都是自己的主角，Sonder520 是主角的工具。"