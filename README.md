...
## 功能
- 首页总览：问候语 + 每日金句、今日摘要、快速备忘、七模块概览卡片
...
- 设计计划：灵感收集 + 设计项目与阶段
- 娱乐游戏：井字棋 / 五子棋（AI 对决与双人对弈、简单/普通/困难三档 AI 难度、悔棋、认输、自动战绩）
- 数据与设置：主题、壁纸透明度、动画帧率（60/90/120）、模块开关、统计、导出/导入备份、迁移至 IndexedDB
- 可靠性：PWA 离线可用、存储双写双存（localStorage + IndexedDB）、存储超 4.5MB 顶部警示条、全局搜索、全模块 XSS 净化
...
- 运行全部测试：`npm test`（当前 184 项全部通过，覆盖存储/UI/各模块/样式/动效/壁纸/移动端/性能/游戏引擎/交互质量回归/PWA/搜索/XSS 净化/IndexedDB）。
- 类型检查（零构建）：`npm run typecheck`（JSDoc + tsc --noEmit）。

## 文档

- 产品需求：见 [PRD.md](PRD.md)（含视觉设计、验收标准与技术架构说明）。
