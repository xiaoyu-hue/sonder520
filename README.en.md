...
- Run all tests: npm test (currently 561 passing, covering storage/encryption (incl. races & future-version resilience)/structured storage status (TrustLayer contract)/standard module factory (ModuleFactory contracts & behavior)/UI/all modules/styles/animations/wallpaper/mobile adaptation/perf/game engines/interaction regressions/PWA/search/XSS (incl. attribute injection)/contrast/IndexedDB/notifications/Web Worker/crisis fallback/motion-layer contracts & behavior).
