...
/* 集合级持久化：逐集合 key 落盘（旧整份 STORAGE_KEY 仅 legacy 迁移源） */
...
test('写锁：单标签正常写后基线一致，不误判让位', () => {
  ...
  try {
    ...
    runQueued();
    const raw = JSON.parse(storage.getItem(COL('memos')));
    assert.equal(raw.length, 2, '连续两次写均正常落盘');
    assert.equal(raw[0].text, 'B');
...
test('写锁：另一标签已写更新（meta 不一致）→ 让位不覆盖 + 吸收新快照 + 广播 all', async () => {
  ...
  try {
    ...
    /* 模拟另一标签抢先写入：meta 变为其他值，data 为更新快照（集合级：另一标签只写了 memos 集合） */
    ...
    assert.equal(stored[0].text, '另一标签新数据', '让位：不得用旧快照覆盖更新数据');
...
test('写锁：锁异常（request reject）→ 降级直接落盘，数据不丢', async () => {
  ...
  try {
    ...
    const raw = JSON.parse(storage.getItem(COL('memos')));
    assert.equal(raw.length, 1, '锁不可用时直接落盘（可用性优先）');
  } finally { clearLocks(); }
});
...
test('写锁：无 navigator.locks 环境 → 直接落盘（等价旧行为）', () => {
  ...
  runQueued();
  const raw = JSON.parse(storage.getItem(COL('memos')));
  assert.equal(raw.length, 1, '无锁直接落盘');
  assert.ok(storage.getItem(S.STORAGE_META_KEY), 'meta 一并写入');
});
...
function encRaw(storage) {
  /* 集合级密文：memos 集合必有（本测试只写备忘） */
  const raw = storage.getItem(COL('memos'));
  return raw ? JSON.parse(raw) : null;
}

/* 用存储中的盐 + 密码派生密钥，把 arr（集合数组）加密成可放回该集合 key 的密文 payload（模拟另一标签落盘） */
async function encryptCol(storage, arr) {
  const salt = CRYPTO.b64ToBytes(storage.getItem(SALT_KEY));
  const key = await CRYPTO.deriveKey(PWD, salt);
  const bundle = await CRYPTO.encryptText(JSON.stringify(arr), key);
  return JSON.stringify({ e: 1, v: bundle.v, iv: bundle.iv, data: bundle.data });
}

/* 解密 memos 集合密文，返回数组 */
async function decryptCol(storage) {
...
test('加密态写锁：锁内 meta 一致 → 正常加密落盘，不误报让位', async () => {
  ...
  try {
    ...
    const memos = await decryptCol(storage);
    assert.ok(memos.some(m => m.text === '加密后新数据'), '加密态新数据已落盘');
    assert.ok(memos.some(m => m.text === '明文基线'), '旧数据保留');
    assert.equal(storage.getItem(S.STORAGE_META_KEY), s._meta, '落盘 meta 与实例一致');
...
test('加密态写锁：另一标签已写更新密文（meta 不一致）→ 让位不覆盖 + 吸收 + 广播', async () => {
  ...
  try {
    ...
    /* 模拟另一标签使用同一密码写入更新密文快照（集合级：只写 memos 集合） */
    const otherState = JSON.parse(JSON.stringify(s.state));
    otherState.memos.unshift({ id: 'other-1', text: '另一标签新数据', time: '2026-08-16T00:00:00.000Z', archived: false });
    storage.setItem(S.STORAGE_META_KEY, 'META-OTHER');
    storage.setItem(COL('memos'), await encryptCol(storage, otherState.memos));
    ...
    assert.equal(storage.getItem(S.STORAGE_META_KEY), 'META-OTHER', 'meta 不被回写');
    const memos = await decryptCol(storage);
    assert.equal(memos[0].text, '另一标签新数据', '让位：密文快照未被本标签旧数据覆盖');
    assert.ok(!memos.some(m => m.text === '本标签新输入'), '本次未落盘（让位放弃）');
...
test('加密态写锁：无 navigator.locks → 降级直接落盘（等价旧行为）', async () => {
  ...
  assert.ok(memos.some(m => m.text === '无锁加密新数据'), '无锁环境加密态直接落盘');
});