...
const mobile720 = css.slice(css.lastIndexOf('@media (max-width: 720px)')).split('@media (max-width: 360px)')[0];
...
test('移动端导航：≤720px 时侧栏变为底部玻璃导航栏', () => {
  assert.ok(/@media \(max-width: 720px\)/.test(mobile720), '缺 720px 断点');
  assert.ok(mobile720.includes('flex-direction: row'), '侧栏应横向');
  assert.ok(mobile720.includes('order: 2') || mobile720.includes('flex: 0 0 auto'), '主区域在上、导航在下');
  assert.ok(mobile720.includes('env(safe-area-inset-bottom'), '底部应适配安全区');
  assert.ok(mobile720.includes('min-height: 44px'), '触控目标 ≥44px');
  assert.ok(mobile720.includes('.sidebar'), '应复用侧栏结构');
...
test('移动端触控：防 iOS 聚焦缩放、取消点按高亮', () => {
  ...
  assert.ok(mobile720.includes('.field input, .field select, .field textarea { font-size: 16px; }'), '表单输入移动端 16px');
});
...
test('手机端弹窗为底部抽屉（overlay 底部对齐 + 底部弹入动画）', () => {
  const baseOverlay = css.indexOf('.overlay {');
  const baseModalAnim = css.indexOf('.overlay .modal { animation: popIn');
  assert.ok(baseOverlay >= 0 && baseModalAnim >= 0, '缺基础弹窗规则');
  /* 抽屉规则必须位于基础规则之后（同特异性后定义者胜），否则被覆盖、抽屉永不生效 */
  const drawerAlign = css.indexOf('align-items: flex-end');
  const drawerAnim = css.indexOf('.overlay .modal { animation: popUpIn');
  assert.ok(drawerAlign > baseOverlay && drawerAlign > baseModalAnim, '抽屉对齐规则须在基础规则之后');
  assert.ok(drawerAnim > baseModalAnim, '抽屉动画规则须在基础动画之后');
...
test('全平台高度：100dvh 带 100vh 回退（老 iOS/安卓）', () => {
  ...
  assert.ok(mobile720.includes('height: 100vh; height: 100dvh'), '移动端缺 vh→dvh 回退');
});
...
test('五子棋大棋盘手机端：压缩边距保持方正且不溢出', () => {
  ...
  assert.ok(mobile720.includes('.game-board.big'), '720px 断点内应有五子棋棋盘适配');
  assert.ok(/width: min\(97vw, 480px\)/.test(mobile720), '手机大棋盘应限定在可视宽度内');
  assert.ok(mobile720.includes('gap: 2px'), '手机大棋盘应压缩格距');
