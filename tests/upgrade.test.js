...
/* 集合级持久化：密文/明文落在逐集合 key（seeded 只写 tasks/memos，断言统一读 tasks 集合） */
...
test('启用加密：主存储变为密文，盐独立存放，快照不含明文', async () => {
  ...
  assert.ok(!st._data[COL('tasks')].includes('任务甲'), '密文不得包含明文');
...
test('解锁：错误密码失败且快照完好，正确密码数据一致', async () => {
  ...
  const before = st._data[COL('tasks')];
  const s2 = S.createStore(st);
  assert.equal(await s2.unlock('错误密码'), false, '错密码应失败');
  assert.equal(st._data[COL('tasks')], before, '错密码不得破坏快照');
...
test('解锁后修改数据：落盘仍为密文，且新实例可解锁读取', async () => {
  ...
  assert.ok(!st._data[COL('tasks')].includes('新任务C'), '密文不含新增明文');
  const s2 = S.createStore(st);
...
test('停用加密：密码正确转回明文并清盐，数据完好', async () => {
  ...
  assert.ok(JSON.parse(st._data[COL('tasks')]).e === 1, '错密码后仍为密文');
  await assert.doesNotReject(() => s.disableEncryption(PWD));
  assert.equal(JSON.parse(st._data[COL('tasks')]).e, undefined, '停用后应为明文');
  assert.equal(st._data[SALT_KEY], undefined, '盐应清除');
  assert.ok(st._data[COL('tasks')].includes('甲'), '明文可读');
  const s2 = S.createStore(st);
...
test('导出/导入：加密备份导出为密文包，密码导入成功、错密码拒绝且原数据不动', async () => {
  ...
  assert.ok(st2._data[COL('tasks')].includes('本地原数据'), '导入失败不得动原数据');
...
test('IndexedDB 兜底：localStorage 被清后仍可从 IDB 密文与冗余盐恢复', async () => {
  ...
  try {
    ...
    assert.ok(st2._data[COL('tasks')], '恢复后应回写 localStorage 密文');
  } finally {
...
test('锁定态守卫：lock 后残留 save 不得明文覆盖密文', async () => {
  ...
  assert.ok(!st._data[COL('tasks')].includes('锁定后新增'), '明文不得落盘');
  const s2 = S.createStore(st);
...
test('锁定态导入明文备份被拒绝，原密文完好', async () => {
  ...
  assert.equal(st._data[COL('tasks')], before, '密文快照原样保留');
});
...
test('加密解锁态导入明文备份：落盘保持密文不变量', async () => {
  ...
  assert.ok(!st._data[COL('tasks')].includes('导入的新数据'), '明文不得出现在落盘');
  const s2 = S.createStore(st);
  assert.equal(await s2.unlock(PWD), true);
...
test('加密格式韧性：未来版本密文（v2）不按明文解析、不落盘，数据原样保留', async () => {
  ...
  st._data[COL('tasks')] = rawV2;
  const s3 = S.createStore(st);
  assert.equal(s3.needsUnlock(), true, 'v2 密文应判定需解锁而非解析成空明文');
  s3.addTask({ title: '锁定后新增', priority: '高' });
  assert.ok(st._data[COL('tasks')].includes('sonder-enc-v2'), '锁定态写入不得落盘（未破坏 v2 密文）');
  assert.ok(!st._data[COL('tasks')].includes('锁定后新增'), '明文不得落盘');
});

test('_idbWriteCols 空参守卫：拒绝 undefined/null，绝不回退内存序列化', async () => {
  const { IDBFactory, IDBKeyRange } = require('fake-indexeddb');
  const f = new IDBFactory();
  globalThis.indexedDB = f;
  globalThis.IDBKeyRange = IDBKeyRange;
  try {
    ...
    s._idbWriteCols(undefined);
    s._idbWriteCols(null);
    s._idbWriteCols('');
    await s._idbPromise;
    const s2 = S.createStore(st);
...
test('加密态导入：importBackup 完成即落盘，不依赖调用方再等待 _encChain', async () => {
  ...
  assert.notEqual(st._data[COL('tasks')], before, 'resolve 时落盘必须已更新（不许 fire-and-forget）');
  const raw = JSON.parse(st._data[COL('tasks')]);
  assert.equal(raw.e, 1, '落盘为密文格式');
  assert.ok(!st._data[COL('tasks')].includes('导入即落盘'), '明文不得出现在落盘');
  const s3 = S.createStore(st);
