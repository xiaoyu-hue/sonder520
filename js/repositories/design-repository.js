/* design-repository.js - 设计计划集合的数据访问边界（v6.x Phase 5 提炼）
 *
 * 定位：designs 已走 ModuleFactory（路径 A），本文件将其正式包装为 Repository
 * 接口（get/getAll/create/update/remove），使三页与 6 个直连迁移集合共用
 * 同一数据访问边界形态。内部直接委托工厂模块，行为零变化
 * （commit/事件/undo 语义不变）。
 *
 * 边界规则（执行方案 §11）：只负责数据访问，不掺 UI 逻辑、页面状态、DOM。
 * CONFIG 留在页面（design.js 单源，含 STAGES），本文件为纯包装层。
 *
 * 浏览器：<script src="js/repositories/design-repository.js">（ModuleFactory 之后、design.js 之前）
 * Node：module.exports 返回工厂 createDesignsRepository(store, config)
 */
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory;
  else root.SonderDesignsRepository = factory();
})(typeof self !== 'undefined' ? self : this, function () {
  'use strict';

  function createDesignsRepository(store, config) {
    var F = globalThis.SonderModuleFactory;
    if (!F || typeof F.createModule !== 'function') {
      throw new TypeError('DesignsRepository: 需要 SonderModuleFactory（framework/ModuleFactory.js 先加载）');
    }
  ...
  return { createDesignsRepository: createDesignsRepository };
});
