...
(function () {
  ...
  var CONFIG = {
    ...
    fields: [
      ...
      { key: 'status', type: 'select', label: '状态', options: [{ value: 'draft', label: '草稿' }, { value: 'queue', label: '排队' }, { value: 'published', label: '已发布' }] },
  ...
  function openAdd(ctx, target) {
    ctx.UI.formModal({
      ...
      fields: [
        ...
        { key: 'status', label: '状态', type: 'select', value: target ? target.status : 'draft', options: [{ value: 'draft', label: '草稿' }, { value: 'queue', label: '排队' }, { value: 'published', label: '已发布' }] },
