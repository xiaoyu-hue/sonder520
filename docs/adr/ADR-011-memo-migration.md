...
依据 docs/plans/plan-framework-review.md 四查结论（只读评估，无代码变更）：
...
- 结论:**Phase 7 标准模块收官——八个标准模块(memo/today/dev/news/selfmedia/consulting/reading/design)全部迁入工厂**,零扩展轨迹延续八试点,factory 能力边界冻结于 v0.1.2;type/stage 等业务字段声明为 select 的 sanitize 通道做法是 type 字段集合的通用迁移形态(select 白名单可等价领域归一规则)。剩余 design 外无标准模块。无新待办引入。
- 演进补充(2026-08-20):storageKey 粒度持久化(ADR-009 决策 7)已落地(见 ADR-009 决策 7 更新与 `docs/plans/plan-storage-key-granularity.md`)——工厂/领域 CRUD 落盘收口 `store._commit(集合id)`,逐集合独立 key(LS `sonder_col_<id>_v1` / IDB entry key `<id>`),八模块集合不再经整份主快照读写;旧整份 `STORAGE_KEY` 仅作 legacy 迁移源,旧 key 保留不删。测试 584 → 592,缓存 v47 → v48。
