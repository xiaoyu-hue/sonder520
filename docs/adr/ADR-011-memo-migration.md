...
- 演进补充(2026-08-20):storageKey 粒度持久化(ADR-009 决策 7)已落地(见 ADR-009 决策 7 更新与 `docs/plan-storage-key-granularity.md`)——工厂/领域 CRUD 落盘收口 `store._commit(集合id)`,逐集合独立 key(LS `sonder_col_<id>_v1` / IDB entry key `<id>`),八模块集合不再经整份主快照读写;旧整份 `STORAGE_KEY` 仅作 legacy 迁移源,旧 key 保留不删。测试 584 → 592,缓存 v47 → v48。
