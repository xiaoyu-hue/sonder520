...
**防回潮契约**：tests/responsive.test.js 升级为断点集合契约——解析 css/*.css 全部 @media 值，宽度类值断言 ⊆ 允许集合 `{360, 720, 960, 1240, 900×480 组合}`，新增魔法数即红；特性查询（`prefers-reduced-motion` / `(hover: hover)` / `(pointer: fine)`）不属于宽度断点，豁免于白名单但须逐条列出登记。
