...
- 现状基线：695 项测试全绿、232 次提交、13 份 ADR（ADR-013 写路径收口 / ADR-014 Phase④ IndexedDB 真源反转已落地）、双站线上（CF Pages + GH Pages，缓存 v71）、IndexedDB 主快照 + localStorage 副本（双写双存，集合级逐 key；IDB=载入真源，LS=跨标签信号+副本）、可选 AES-GCM 加密（逐集合 bundle）、PWA、Web Worker、12 个 hash 路由模块。
