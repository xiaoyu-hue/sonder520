...
- 现状基线：538 项测试全绿、164 次提交、8 份 ADR、双站线上（CF Pages + GH Pages，缓存 v34）、IndexedDB 主快照 + localStorage 副本（双写双存）、可选 AES-GCM 加密、PWA、Web Worker、12 个 hash 路由模块。
...
- **现有 538 项测试是迁移安全网，不是旧包袱**——不得因"更干净"而大规模删除；旧测试验证行为则保留，仅验证废弃内部实现才可重写。
